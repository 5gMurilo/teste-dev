...
interface ViaCepProviderResponse {
  cep: string;
  logradouro: string;
  bairro: string;
  cidade: string;
...
export class ViaCepProvider implements CepProvider {
  ...
  constructor(
    ...
    this.baseUrl = this.config.get<string>("providers.ViaCepProvider.baseUrl")!;
  ...
  private readonly CEP_PROVIDER_KEY = "Via-CEP";

  async findByCep(cep: string): Promise<CepAddress> {
    return this.breaker.execute(
      async () => {
        try {
          const response = await this.http.get<ViaCepProviderResponse>(
            `${this.baseUrl}${cep}/json`,
            {
              provider: this.CEP_PROVIDER_KEY,
              timeoutMs: this.config.get<number>(
                "providers.ViaCepProvider.timeoutMs",
              ),
            },
          );

          if (response.erro) {
            throw new CepNotFoundError(this.CEP_PROVIDER_KEY);
          }
